  ...
  const renderModeToggle = () => {
    ...
    return (
      <button
        ...
        className="px-2.5 py-1 rounded-full text-[10px] font-semibold flex items-center gap-1.5 transition-all cursor-pointer border hover:shadow-sm shrink-0"
        style={{
  ...
  return (
    <div className="flex h-dvh overflow-hidden" style={{ background: "var(--background)" }}>
      ...
      <div className="flex-1 flex flex-col h-full min-w-0 relative">
        ...
        {/* Main Header */}
        <header
          className="flex flex-col border-b shrink-0 shadow-sm"
          style={{ borderColor: "var(--border)", background: "var(--card)" }}
        >
          <div className="px-4 sm:px-6 py-3 sm:py-4 flex items-center justify-between gap-2">
            <div className="flex items-center gap-2 sm:gap-3 flex-1 min-w-0">
              <button
                ...
                className="sm:hidden shrink-0 p-1.5 rounded-lg border hover:bg-gray-50 transition-colors cursor-pointer"
                style={{ borderColor: "var(--border)", color: "var(--muted)" }}
              >
                <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              ...
              <span
                className="text-base sm:text-lg font-semibold tracking-tight truncate min-w-0 flex-1 sm:flex-initial sm:max-w-xs"
                style={{ color: "var(--foreground)" }}
              >
                {activeSession ? activeSession.title : "新しい相談室"}
              </span>
              {/* Mode toggle: inline with the title on tablet/desktop, moves to its own row on mobile */}
            ...
            <div className="flex items-center gap-2 shrink-0">
              ...
              <Link
                href="/profile"
                title="プロフィール"
                className="text-xs px-2.5 sm:px-3 py-2 rounded-lg font-medium border transition-colors hover:opacity-95 flex items-center gap-1.5"
                style={{
                  color: "var(--accent)",
                  borderColor: "var(--accent)",
                  background: "var(--card)",
          ...
          {/* Mode toggle: its own row on mobile so it never squeezes the title */}
          {activeSession && (
